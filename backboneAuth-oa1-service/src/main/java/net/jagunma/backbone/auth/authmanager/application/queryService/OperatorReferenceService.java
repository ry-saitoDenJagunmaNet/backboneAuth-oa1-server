...
import net.jagunma.backbone.auth.authmanager.model.types.AccountLockLockStatus;
...
import net.jagunma.backbone.auth.authmanager.model.types.PasswordHistoryChangeType;
...
public class OperatorReferenceService {
    ...
    private List<OperatorReferenceDto> getOperatorList(OperatorSearchRequest request) {
        ...
        for (OperatorEntity operatorEntity : operatorEntities) {
            ...
            if (!setOperatorBizTranRoleInfo(dto, request, operatorEntity,
                operatorBizTranRoles.getValues())) {
                continue;
            }
    ...
    private boolean setOperatorSubSystemRoleInfo(OperatorReferenceDto dto,
        ...
        List<Oa11010SearchConverterOperatorSubSystemRole> requestOperatorSubSystemRoles =
            request.getSubSystemRoleList().stream().filter(reqossr ->
                Oa11010Vo.CHECKBOX_TRUE.equals(reqossr.getSubSystemRoleSelected()))
                .collect(Collectors.toList());
        if (requestOperatorSubSystemRoles.size() > 0) {
            ...
            List<Operator_SubSystemRole> operatorSubSystemRolefilter = operatorSubSystemRoles
                .stream().filter(ossr -> ossr.getOperatorId() == operatorEntity.getOperatorId())
                .collect(Collectors.toList());
            ...
            if (ConditionsSelect.ＡＮＤ.getCode().equals(request.getSubSystemRoleConditionsSelect())) {
                ...
                for (Oa11010SearchConverterOperatorSubSystemRole requestOperatorSubSystemRole : requestOperatorSubSystemRoles) {
                    if (getFilterOperatorSubSystemRoleDtoList(operatorSubSystemRolefilter,
                        requestOperatorSubSystemRole).size() == 0) {
                        System.out.println("### OperatorId=" + operatorEntity.getOperatorId());
                        return false;
                    }
                }
            } else if (ConditionsSelect.ＯＲ.getCode()
                .equals(request.getSubSystemRoleConditionsSelect())) {
                // OR
     ...
     */
    private List<Operator_SubSystemRole> getFilterOperatorSubSystemRoleDtoList(
        List<Operator_SubSystemRole> operatorSubSystemRoles,
        Oa11010SearchConverterOperatorSubSystemRole requestOperatorSubSystemRole) {

        List<Operator_SubSystemRole> filters = newArrayList();
        if (ConditionsExpirationSelect.指定なし.getCode()
            .equals(requestOperatorSubSystemRole.getExpirationSelect())) {
            // 指定無し
            filters = operatorSubSystemRoles.stream().filter(ossr ->
                ossr.getSubSystemRoleCode()
                    .equals(requestOperatorSubSystemRole.getSubSystemRoleCode()))
                .collect(Collectors.toList());
        } else if (ConditionsExpirationSelect.状態指定日.getCode()
            .equals(requestOperatorSubSystemRole.getExpirationSelect())) {
            // 状態指定日
            filters = operatorSubSystemRoles.stream().filter(ossr ->
                ossr.getSubSystemRoleCode()
                    .equals(requestOperatorSubSystemRole.getSubSystemRoleCode()) &&
                    ossr.getExpirationStartDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationStatusDate()) <= 0 &&
                    ossr.getExpirationEndDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationStatusDate()) >= 0)
                .collect(Collectors.toList());
        } else if (ConditionsExpirationSelect.条件指定.getCode()
            .equals(requestOperatorSubSystemRole.getExpirationSelect())) {
            // 条件指定
            filters = operatorSubSystemRoles.stream().filter(ossr ->
                ossr.getSubSystemRoleCode()
                    .equals(requestOperatorSubSystemRole.getSubSystemRoleCode()) &&
                    (ossr.getExpirationStartDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationStartDateFrom()) >= 0)
                    &&
                    ossr.getExpirationStartDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationStartDateTo()) <= 0 &&
                    ossr.getExpirationEndDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationEndDateFrom()) >= 0 &&
                    ossr.getExpirationEndDate()
                        .compareTo(requestOperatorSubSystemRole.getExpirationEndDateTo()) <= 0)
                .collect(Collectors.toList());
        }
        return filters;
    }
    ...
    private boolean setOperatorBizTranRoleInfo(OperatorReferenceDto dto,
        ...
        List<Oa11010SearchConverterOperatorBizTranRole> requestOperatorBizTranRoles =
            request.getBizTranRoleList().stream().filter(reqobtr ->
                Oa11010Vo.CHECKBOX_TRUE.equals(reqobtr.getBizTranRoleSelected()))
                .collect(Collectors.toList());
        if (requestOperatorBizTranRoles.size() > 0) {
            ...
            List<Operator_BizTranRole> operatorBizTranRoleListFilter = operatorBizTranRoleList
                .stream().filter(obtrl -> obtrl.getOperatorId() == operatorEntity.getOperatorId())
                .collect(Collectors.toList());
            ...
            if (ConditionsSelect.ＡＮＤ.getCode().equals(request.getBizTranRoleConditionsSelect())) {
                ...
                for (Oa11010SearchConverterOperatorBizTranRole requestOperatorBizTranRole : requestOperatorBizTranRoles) {
                    if (getFilterOperatorBizTranRoleDtoList(operatorBizTranRoleListFilter,
                        requestOperatorBizTranRole).size() == 0) {
                        return false;
                    }
                }
            } else if (ConditionsSelect.ＯＲ.getCode()
                .equals(request.getBizTranRoleConditionsSelect())) {
                // OR
                boolean orReturn = false;
                for (Oa11010SearchConverterOperatorBizTranRole requestOperatorBizTranRole : requestOperatorBizTranRoles) {
                    if (getFilterOperatorBizTranRoleDtoList(operatorBizTranRoleListFilter,
     ...
     */
    private List<Operator_BizTranRole> getFilterOperatorBizTranRoleDtoList(
        List<Operator_BizTranRole> operatorBizTranRoleList,
        Oa11010SearchConverterOperatorBizTranRole requestOperatorBizTranRole) {

        List<Operator_BizTranRole> filters = newArrayList();
        if (ConditionsExpirationSelect.指定なし.getCode()
            .equals(requestOperatorBizTranRole.getExpirationSelect())) {
            // 指定無し
            filters = operatorBizTranRoleList.stream().filter(obtr ->
                obtr.getBizTranRole().getBizTranRoleCode()
                    .equals(requestOperatorBizTranRole.getBizTranRoleCode()))
                .collect(Collectors.toList());
        } else if (ConditionsExpirationSelect.状態指定日.getCode()
            .equals(requestOperatorBizTranRole.getExpirationSelect())) {
            // 状態指定日
            System.out
                .println("### StatusDate=" + requestOperatorBizTranRole.getExpirationStatusDate() +
                    ",BizTranRoleCode=" + requestOperatorBizTranRole.getBizTranRoleCode());
            filters = operatorBizTranRoleList.stream().filter(obtr ->
                obtr.getBizTranRole().getBizTranRoleCode()
                    .equals(requestOperatorBizTranRole.getBizTranRoleCode()) &&
                    obtr.getExpirationStartDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationStatusDate()) <= 0 &&
                    obtr.getExpirationEndDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationStatusDate()) >= 0)
                .collect(Collectors.toList());
        } else if (ConditionsExpirationSelect.条件指定.getCode()
            .equals(requestOperatorBizTranRole.getExpirationSelect())) {
            // 条件指定
            System.out.println(
                "### StartDateFrom=" + requestOperatorBizTranRole.getExpirationStartDateFrom() +
            ...
            filters = operatorBizTranRoleList.stream().filter(obtr ->
                obtr.getBizTranRole().getBizTranRoleCode()
                    .equals(requestOperatorBizTranRole.getBizTranRoleCode()) &&
                    (obtr.getExpirationStartDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationStartDateFrom()) >= 0) &&
                    obtr.getExpirationStartDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationStartDateTo()) <= 0 &&
                    obtr.getExpirationEndDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationEndDateFrom()) >= 0 &&
                    obtr.getExpirationEndDate()
                        .compareTo(requestOperatorBizTranRole.getExpirationEndDateTo()) <= 0)
                .collect(Collectors.toList());
        }
        return filters;
    }
    ...
    private boolean setAccountLockInfo(OperatorReferenceDto dto,
        ...
        // アカウントロックの項目設定
        List<AccountLockEntity> asccountLocks = accountLockEntities.stream()
            .filter(a -> a.getOperatorId().equals(operatorEntity.getOperatorId()))
            .collect(Collectors.toList());
        ...
        if (request.getAccountLockOccurredDateFrom() != null ||
            ...
            if (asccountLocks.size() == 0) {
                ...
                if (request.getAccountLockOccurredDateFrom() != null &&
                    dto.getOccurredDateTime()
                        .compareTo(request.getAccountLockOccurredDateFrom().atStartOfDay()) < 0) {
                    //continue;
                    return false;
                }
                if (request.getAccountLockOccurredDateTo() != null &&
                    dto.getOccurredDateTime().compareTo(
                        request.getAccountLockOccurredDateTo().plusDays(1).atStartOfDay()) > 0) {
                    //continue;
                    return false;
                }
            }
        ...
        if (request.getAccountLockStatusLock() != null) {
            accountLockStatusLock = request.getAccountLockStatusLock();
        }
        if (request.getAccountLockStatusUnlock() != null) {
            accountLockStatusUnlock = request.getAccountLockStatusUnlock();
        }
    ...
    private boolean setPasswordHistoryInfo(OperatorReferenceDto dto,
        ...
        // パスワード履歴の項目設定
        List<PasswordHistoryEntity> passwordHistorys = passwordHistoryEntities.stream()
            .filter(a -> a.getOperatorId().equals(operatorEntity.getOperatorId()))
            .collect(Collectors.toList());
        ...
        if (Oa11010Vo.CHECKBOX_TRUE.equals(request.getPasswordHistoryCheck()) &&
            ...
            LocalDate passwodrChanheDate = LocalDate.now()
                .minusDays(request.getPasswordHistoryLastChangeDate());
        ...
        if (request.getPasswordHistoryChangeType0() != null) {
            passwordHistoryChangeType0 = request.getPasswordHistoryChangeType0();
        }
        if (request.getPasswordHistoryChangeType1() != null) {
            passwordHistoryChangeType1 = request.getPasswordHistoryChangeType1();
        }
        if (request.getPasswordHistoryChangeType2() != null) {
            passwordHistoryChangeType2 = request.getPasswordHistoryChangeType2();
        }
        if (request.getPasswordHistoryChangeType3() != null) {
            passwordHistoryChangeType3 = request.getPasswordHistoryChangeType2();
        }
        if (!passwordHistoryChangeType0.equals(passwordHistoryChangeType1) ||
            ...
            // 最終パスワード変更種別が全て同じでない
            if (PasswordHistoryChangeType.初期.getCode().equals(dto.getChangeType())
                && !Oa11010Vo.CHECKBOX_TRUE.equals(passwordHistoryChangeType0)) {
                // 初期
                return false;
            } else if (PasswordHistoryChangeType.ユーザーによる変更.getCode().equals(dto.getChangeType())
                && !Oa11010Vo.CHECKBOX_TRUE.equals(passwordHistoryChangeType1)) {
                // ユーザーによる変更
                return false;
            } else if (PasswordHistoryChangeType.管理者によるリセット.getCode().equals(dto.getChangeType())
                && !Oa11010Vo.CHECKBOX_TRUE.equals(passwordHistoryChangeType2)) {
                // 管理者によるリセット
                return false;
            } else if (PasswordHistoryChangeType.機器認証パスワード.getCode().equals(dto.getChangeType())
                && !Oa11010Vo.CHECKBOX_TRUE.equals(passwordHistoryChangeType3)) {
                // 機器認証パスワード
                return false;
            }
        }
    ...
    private boolean setSignInTraceInfo(OperatorReferenceDto dto,
        ...
        // サインイン証跡の項目設定
        List<SignInTraceEntity> signInTraces = signInTraceEntities.stream()
            .filter(a -> a.getOperatorCode().equals(operatorEntity.getOperatorCode()))
            .collect(Collectors.toList());
        ...
        if (request.getSignintraceTrydateFrom() != null ||
            ...
            if (request.getSignintraceTrydateFrom() != null &&
                dto.getTryDateTime().compareTo(request.getSignintraceTrydateFrom().atStartOfDay())
                    < 0) {
                return false;
            }
            if (request.getSignintraceTrydateTo() != null &&
                dto.getTryDateTime()
                    .compareTo(request.getSignintraceTrydateTo().plusDays(1).atStartOfDay()) > 0) {
                return false;
            }
        }
        // サインイン証跡　最終サインイン元IPアドレスによる抽出
        if (request.getSignintraceTryIpAddress() != null
            && request.getSignintraceTryIpAddress().length() > 0) {
            if (signInTraces.size() == 0) {
                return false;
            }
        ...
        if (request.getSignintraceSignInResult() != null
            && request.getSignintraceSignInResult().length > 0) {
            if (signInTraces.size() == 0) {
                return false;
            }
            // 配列をリストに変換
    ...
    private boolean setSignOutTraceInfo(OperatorReferenceDto dto,
        ...
        // サインアウト証跡の項目設定
        List<SignOutTraceEntity> signOutTraces = signOutTraceEntities.stream()
            .filter(a -> a.getOperatorId().equals(operatorEntity.getOperatorId()))
            .collect(Collectors.toList());
