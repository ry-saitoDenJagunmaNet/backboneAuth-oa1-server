...
class Oa12060SearchPresenter implements CalendarSearchResponse {
    ...
    public void bindTo(Oa12060SearchResponseVo vo) {
        ...
        if (calendarTypeFilterCheck1disabled && calendarTypeFilterCheck2disabled
            && calendarTypeFilterCheck3disabled) {
    ...
    public void bindTo(Oa12060SearchResponseVo vo, Oa12060Vo oa12060Vo) {
        ...
        oa12060Vo
            .setCalendarTypeFilterCheck1(calendarTypeFilterCheck1disabled ? (short) 0 : (short) 1);
        oa12060Vo
            .setCalendarTypeFilterCheck2(calendarTypeFilterCheck2disabled ? (short) 0 : (short) 1);
        oa12060Vo
            .setCalendarTypeFilterCheck3(calendarTypeFilterCheck3disabled ? (short) 0 : (short) 1);
    }

    private boolean isCalendarTypeFilterCheckDisabled(String calendarTypeString) {
        if (calendars.getValues().stream()
            .filter(c -> CalendarType.valueOf(calendarTypeString).equals(c.getCalendarType()))
            .count()
    ...
    private String genCalendarTableHtml() {
        ...
        System.out.println(
            "### startDate=" + startDate + ",lastDay=" + lastDay + ",startWeek=" + startWeek);
        ...
        for (int day = 1; day <= lastDay; day++) {
            ...
            html.append(genCalendarCellHtml(day, currentWeek,
                list.stream().filter(c -> c.getDate().equals(targetDate)).collect(
                    Collectors.toList())));
            currentWeek++;
            if (currentWeek >= 7) {
                html.append("  </tr>");
...
//			}
        ...
        html.append(String.format(
            "     <input type=\"checkbox\" id=\"calendar_type_check1[%d]\" name=\"CalendarList[%d].isWorkingDay1\" value=\"1\"%s></input>",
            index, index, checked[0]));
        html.append("     <span>経済</span>");
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].calendarId1\" value=\"%s\"></input>",
            index, calendarId[0]));
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].recordVersion1\" value=\"%d\"></input>",
            index, recordVersion[0]));
        html.append("    </label></div>");

        html.append("    <div><label class=\"oaex_calendarcell_check2\">");
        html.append(String.format(
            "     <input type=\"checkbox\" id=\"calendar_type_check2[%d]\" name=\"CalendarList[%d].isWorkingDay2\" value=\"1\"%s></input>",
            index, index, checked[1]));
        html.append("     <span>信用</span>");
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].calendarId2\" value=\"%s\"></input>",
            index, calendarId[1]));
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].recordVersion2\" value=\"%d\"></input>",
            index, recordVersion[1]));
        html.append("    </label></div>");

        html.append("    <div><label class=\"oaex_calendarcell_check3\">");
        html.append(String.format(
            "     <input type=\"checkbox\" id=\"calendar_type_check3[%d]\" name=\"CalendarList[%d].isWorkingDay3\" value=\"1\"%s></input>",
            index, index, checked[2]));
        html.append("     <span>広域物流</span>");
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].calendarId3\" value=\"%s\"></input>",
            index, calendarId[2]));
        html.append(String.format(
            "     <input type=\"hidden\" name=\"CalendarList[%d].recordVersion3\" value=\"%d\"></input>",
            index, recordVersion[2]));
        html.append("    </label></div>");
        html.append("   </td>");
