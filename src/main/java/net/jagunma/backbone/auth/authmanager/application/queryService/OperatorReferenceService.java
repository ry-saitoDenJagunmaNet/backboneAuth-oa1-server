...
public class OperatorReferenceService {
	...
	public List<OperatorDto> getOperatorList(OperatorSearchRequest request) {
		...
		operatorEntitys.forEach(o -> {
			...
			// 店舗名
			List<TempoDto> tempoDtos =  tempos.stream().filter(t->t.getTempoCode().equals(o.getTempoCode())).collect(Collectors.toList());
			if (tempoDtos.size() > 0) {
				entity.setTempoName(tempoDtos.get(0).getTempoName());
			}
			// ロック状態
			List<AccountLockEntity> asccountLocks = accountLockEntitys.stream().filter(a->a.getOperatorId().equals(o.getOperatorId())).collect(Collectors.toList());
			if (asccountLocks.size() > 0) {
				entity.setLockStatus(asccountLocks.get(0).getAccountLockId().intValue());
			} else {
			...
			entity.setOperatorSubSystemRoleList(
				operatorSubSystemRoles.stream().filter(ossr->ossr.getOperatorId() == o.getOperatorId()).collect(Collectors.toList()));
			// オペレーター_取引ロール割当リスト
			entity.setOperatorBizTranRoleList(
				operatorBizTranRoles.stream().filter(obtr->obtr.getOperatorId() == o.getOperatorId()).collect(Collectors.toList()));
